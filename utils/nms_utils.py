...
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.5, iou_thresh=0.5):
    ...
    boxes = tf.concat(boxes_list, axis=0, name='result/boxes')
    score = tf.concat(score_list, axis=0, name='result/score')
    label = tf.concat(label_list, axis=0, name='result/label')
